...
export default function ResourceAllocationPage() {
  ...
  const [facilityResources, setFacilityResources] = useState<FacilityResource[]>([])
      ...
      setResourceRequests([
        {
          ...
          department: "Computer Science",
          description: "Additional mentor needed for AI/ML specialization to handle increased internship requests",
          priority: "HIGH",
          status: "PENDING",
        ...
        },
        {
          ...
          department: "Chemical Engineering",
          description: "Need access to advanced spectroscopy lab for research internship projects",
          priority: "MEDIUM",
          status: "PENDING",
        ...
        },
        {
          ...
          department: "Mechanical Engineering",
          description: "3D printer for prototyping projects - current equipment at capacity",
          priority: "MEDIUM",
          status: "APPROVED",
        ...
        },
        {
          ...
          department: "Electrical Engineering",
          description: "Additional budget allocation for extended internship program duration",
          priority: "LOW",
          status: "PENDING",
  ...
  const filteredRequests = resourceRequests.filter(req => {
    const typeMatch = filterType === "all" || req.type.toLowerCase() === filterType.toLowerCase()
  ...
  const utilizationData = budgetAllocations.map(dept => ({
    ...
    mentors: mentorResources.filter(m => m.department === dept.department).length,
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Manage and optimize L&D resources, capacity, and budget allocation</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{resourceMetrics?.mentorUtilization || 0}%</p>
                  <p className="text-xs text-gray-500 mt-1">
                    {resourceMetrics?.usedCapacity || 0}/{resourceMetrics?.totalCapacity || 0} capacity
                  </p>
                </div>
                <Users className="h-8 w-8 text-blue-600" />
              </div>
              <div className="mt-4">
                <Progress value={resourceMetrics?.mentorUtilization || 0} className="h-2" />
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">{resourceMetrics?.facilitiesUtilization || 0}%</p>
              ...
              <div className="mt-4">
                <Progress value={resourceMetrics?.facilitiesUtilization || 0} className="h-2" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {resourceMetrics ? Math.round((resourceMetrics.budgetUsed / resourceMetrics.budgetAllocated) * 100) : 0}%
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    ₹{((resourceMetrics?.budgetUsed || 0) / 100000).toFixed(1)}L / ₹{((resourceMetrics?.budgetAllocated || 0) / 100000).toFixed(1)}L
                  </p>
                </div>
              ...
              <div className="mt-4">
                <Progress 
                  value={resourceMetrics ? (resourceMetrics.budgetUsed / resourceMetrics.budgetAllocated) * 100 : 0} 
                  className="h-2" 
                />
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600">{resourceMetrics?.equipmentAvailability || 0}%</p>
                  <p className="text-xs text-orange-600 mt-1">
                    {resourceRequests.filter(r => r.type === 'EQUIPMENT' && r.status === 'PENDING').length} pending requests
                  </p>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Resource allocation and usage across departments</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={utilizationData}>
                  ...
                  <Bar dataKey="budget" fill="#22c55e" name="Budget Utilization %" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Current mentor capacity and availability</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <div className="flex items-center space-x-4 mt-2">
                        <div className="flex-1">
                          <div className="flex justify-between text-xs mb-1">
                            <span>Capacity</span>
                            <span>{mentor.currentLoad}/{mentor.maxCapacity}</span>
                          </div>
                          <Progress value={mentor.utilizationRate} className="h-2" />
                        </div>
                      </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Lab and facility utilization status</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {facilityResources.map((facility) => (
                  <div key={facility.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">{facility.type} • {facility.department || 'Shared'}</p>
                      <div className="flex items-center space-x-4 mt-2">
                        <div className="flex-1">
                          <div className="flex justify-between text-xs mb-1">
                            <span>Utilization</span>
                            <span>{facility.currentBookings}/{facility.capacity}</span>
                          </div>
                      ...
                      {facility.nextAvailable && (
                        <p className="text-xs text-gray-500 mt-1">
                          Next available: {new Date(facility.nextAvailable).toLocaleDateString()}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center space-x-2 ml-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Financial resource distribution and utilization</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {budgetAllocations.map((budget, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>₹{(budget.allocated / 100000).toFixed(1)}L</TableCell>
                    <TableCell>₹{(budget.used / 100000).toFixed(1)}L</TableCell>
                    <TableCell>
                      <span className={budget.remaining < budget.allocated * 0.2 ? "text-red-600" : "text-green-600"}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <Progress value={budget.utilizationRate} className="w-16 h-2" />
                      </div>
                    </TableCell>
                    <TableCell>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Pending and recent resource allocation requests</CardDescription>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>AI-powered suggestions for resource optimization</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <Alert className="border-blue-200 bg-blue-50">
                ...
                <AlertDescription className="text-blue-700">
                  <strong>Mentor Reallocation:</strong> Consider redistributing mentors from Chemical Engineering 
                  (100% utilization) to Computer Science (50% utilization) to balance workload.
                </AlertDescription>
              </Alert>
              <Alert className="border-yellow-200 bg-yellow-50">
                <AlertTriangle className="h-4 w-4 text-yellow-600" />
                <AlertDescription className="text-yellow-700">
                  <strong>Budget Alert:</strong> Mechanical Engineering department is at 85% budget utilization. 
                  Consider reallocating funds from under-utilized departments.
                </AlertDescription>
              </Alert>
              <Alert className="border-green-200 bg-green-50">
                <CheckCircle className="h-4 w-4 text-green-600" />
                <AlertDescription className="text-green-700">
                  <strong>Efficiency Gain:</strong> Conference Room Alpha is underutilized (48%). 
                  This resource can accommodate additional training sessions.
                </AlertDescription>
              </Alert>
            </div>
          </CardContent>
